...
донатом себе на второй кошелёк за оборот суммы плюс `INGEST_PRICE` ≈ $0.019, `00 §10.1`), но теперь она —
...
раунда — `register_task` пруфом рождения **против кешированного корня** (обход хеш-дерева, O(log n);
