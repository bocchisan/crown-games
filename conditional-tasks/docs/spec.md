...
**Отсюда — кто чем рискует, и это надо знать, а не выводить.** Умолчание безопасно для донора: молчание
голосующих есть `Cancel`, деньги возвращаются, и ончейн-`refund()` работает без канистры. Риск лежит на
**получателе**: без кворума исход решает тот, кто пришёл, а прийти стоит `MIN_VOTE_WEIGHT = $0.10`
репутации у этого получателя — покупаемой прямым донатом себе на второй кошелёк за оборот суммы плюс
`INGEST_PRICE` ≈ $0.019 (`00 §10.1`). То есть один голос «не сделано» за ~$0.12 отменяет уже выполненную
работу, и подать его может сам донор. Это не обход механики — механика ровно такая; вопрос в том,
объявлена ли приёмка работы социальной (репутация сторон публична и линейна) или числа обязаны стоять
против цены покупки веса. Решается до mainnet (`crown-spec/docs/08-deferred.md`, чеклист `P8`); после
запуска дороже — `voting_period` вшит в прообраз `task_id`, а игра заменяется, а не апгрейдится.
