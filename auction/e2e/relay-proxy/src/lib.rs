...
//! verdict", `WrongTarget`, `NotFound` — are reachable only from an inter-canister
//! caller that attaches cycles. This canister is that caller: it forwards raw,
//! pre-encoded args to any method, so one fixture serves every game.
