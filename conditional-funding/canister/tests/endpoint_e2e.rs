...
/// state-changing method takes — a claim about `inspect_message`, not about
/// candid, so it is worth a test rather than a sentence. Both calls below carry
/// the arguments the method actually declares; they are dropped anyway, because
/// ingress cannot pay and an unpaid pull is work nobody ordered.
#[test]
fn a_direct_ingress_on_the_paid_pulls_is_dropped() {
    ...
    let sig = pic.update_call(
        ...
        Principal::anonymous(),
        "request_signature",
        Encode!(&CHAIN.to_string(), &unknown_collection()).unwrap(),
    );
    assert!(
        sig.is_err(),
...
fn queries_wire_through() {
    ...
    assert_eq!(Decode!(&v, u32).unwrap(), 4); // conditional-funding LOGIC_VERSION
