...
- `donor == recipient` и самоголосование не запрещены — и **«сдерживаются публичностью» здесь надо
  ...
  стоит оборота суммы плюс `INGEST_PRICE` ≈ $0.019 за впись. При `quorum_weight = $1.00` и
  `approval_threshold = 5000` получатель добирает кворум и большинство примерно за $1 оборота и две
  копейки — а решает этот голос судьбу **чужих** вкладов, потому что `settle` уводит их ему. Механика
  тут ни при чём: правило вердикта такое и записано, вклады возвращаются по умолчанию (тишина →
  `refund`), и ончейн-`refund()` остаётся у донора. Вопрос в **числе**: `quorum_weight` обязан стоять
  против цены покупки веса, либо условность сбора объявляется социальной прямым текстом — здесь и на
  витрине. Решается до mainnet (`crown-spec/docs/08-deferred.md`, чеклист `P8`); после запуска дороже —
  `quorum_weight`/`approval_threshold`/`voting_period` вшиты в прообраз `collection_id`, а игра
  заменяется, а не апгрейдится.
